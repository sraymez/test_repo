...
#prec <- worldclim_country(country = "GMB", var = "prec", mask = TRUE)
...
admPlot <-
  tm_shape(adm1) + 
  tm_polygons("ADM1_EN", title = "Admin 1 boundries", palette = "Spectral") + 
  tm_layout(frame = F, legend.outside = T)

popPlot <- 
  tm_shape(pop) + 
  tm_raster(style = "cont", palette = "Spectral", title = "Population density (km^2)") + 
  tm_layout(frame = F)

landPlot <- 
  tm_shape(landcover) + 
  tm_raster(style = "cont", palette = "Spectral", title = "Land cover") + 
  tm_layout(frame = F)
landPlot

elevPlot <- 
  tm_shape(elev) + 
  tm_raster(style = "cont", palette = "-Spectral", title = "Elevation (m)") +
  tm_layout(frame = F)
...
waterBody <- 
  ...
  tm_shape(waterA) +
  tm_polygons(col = "blue", title = "Inland water")
...
rainyPrecPlot <- 
  tm_shape(meanRainyPrec) + 
  tm_raster(style = "cont", palette = "-Spectral", title = "Rainy reason precipitation (mm)")  + 
  tm_layout(frame = F)

dryPrecPlot <- 
  tm_shape(meanDryPrec) + 
  tm_raster(style = "cont", palette = "-Spectral", title = "Dry reason precipitation (mm)")  + 
  tm_layout(frame = F)

tmap_arrange(admPlot, popPlot, landPlot, elevPlot, rainyPrecPlot, 
             dryPrecPlot, roads, waterBody,
             ncol = 3, nrow = 3)
...
# Get bounding box
gmb_sites <- gm_adm2 %>% filter(ADM1_EN %in% c("Central River North", "Central River South"))
st_bbox(gmb_sites)

# Specify land cover modis download
MODIStsp(gui = FALSE,
         out_folder = "Raster/MODIS",
         out_folder_mod = "Raster/MODIS",
         ...
         end_date = "2022.12.31",
         verbose = TRUE,
         bbox = c(-15.46274, 13.32131, -14.44634, 13.82274),
         spatmeth = "bbox",
         out_format = "GTiff",
         compress = "LZW",
...
# Import data
landcover_files_gmb <- list.files("Raster/Modis/LandCover_Type_Yearly_500m_v61_GMB_site/LC1", "MCD12Q1_LC1_2022_001", full.names = T)
...
temp_df_gmb <- 
  as.data.frame(
    cbind(
      landcover_breaks = c(9,10,11,12,13,14,16,17),
      landcover_classes = c("Savannas", "Grasslands", "Permanent wetlands", "Croplands", "Urban and Built-up land",
...
tm_shape(landcover_gmb) +
  tm_raster(title = "Land cover categories") +
  tm_layout(frame = F, legend.position = c(0.75,0.8))

### Nigeria ### 

# Ebonyi 
ebonyi_sites <- tibble(
  name = c("Abakaliki", "Nwenzenyi"),
  lat = c(6.324393096455707, 6.5790414672670146),
  long = c(8.114003412195835, 8.047731519232988))

# Convert to sf object
ebonyi_sites_sf <- ebonyi_sites %>% st_as_sf(coords = c("long", "lat"), crs = 4326)

# Filter shapefile to just ebonyi state
ebonyi <- ng_adm2 %>% filter(NAME_1 == "Ebonyi")
...
MODIStsp(gui = FALSE,
         ...
         end_date = "2022.12.31",
         verbose = TRUE,
         bbox = c(7.525859, 5.690326, 8.444068, 6.802191),
         spatmeth = "bbox",
         out_format = "GTiff",
         compress = "LZW",
         out_projsel = "User Defined",
...
# Import data
landcover_files_ebonyi <- list.files("Raster/Modis/LandCover_Type_Yearly_500m_v61_NGA_E/LC1", "MCD12Q1_LC1_2022_001", full.names = T)
...
temp_df_ebo <- 
  as.data.frame(
    cbind(
      landcover_breaks = c(2,4,5,9,10,12,13,14,17),
      landcover_classes = c("Evergreen Broadleaf Forests", "Deciduous Broadleaf Forests", "Mixed Forests",
                            "Savannas", "Grasslands", "Croplands", "Urban and Built-up land",
...
tm_shape(landcover_ebonyi) +
  tm_raster(title = "Land cover categories") +
  tm_layout(frame = F, legend.position = c(0.8,0), title = "Ebonyi Study Sites") +
  tm_shape(ebonyi) +
  tm_polygons("white", alpha = 0.1) +
  tm_shape(ebonyi_sites_sf) +
...
ondo_sites <- tibble(
  name = c("Owo", "Ogbese"),
  lat = c(7.198907682758365, 7.258109009720957),
  long = c(5.593265586054566, 5.370833374474131))

# Convert to sf object
ondo_sites_sf <- ondo_sites %>% st_as_sf(coords = c("long", "lat"), crs = 4326)

# Filter shapefile to just ondo state
ondo <- ng_adm2 %>% filter(NAME_1 == "Ondo")
...
MODIStsp(gui = FALSE,
         ...
         end_date = "2022.12.31",
         verbose = TRUE,
         bbox = c(4.352340, 5.856250, 6.028376, 7.770609),
         spatmeth = "bbox",
         out_format = "GTiff",
         compress = "LZW",
         out_projsel = "User Defined",
...
# Import data
landcover_files_ondo <- list.files("Raster/Modis/LandCover_Type_Yearly_500m_v61_NGA_O/LC1", "MCD12Q1_LC1_2022_001", full.names = T)
...
temp_df_ond <- 
  as.data.frame(
    cbind(
      landcover_breaks = c(2,4,8,9,10,11,12,13,14,16,17),
      landcover_classes = c("Evergreen Broadleaf Forests", "Deciduous Broadleaf Forests", "Woody Savannas",
                            "Savannas", "Grasslands", "Permanent Wetlands", "Croplands", "Urban and Built-up Lands",
                            "Cropland/Natural Vegetation Mosaics", "Barren", "Water Bodies")))
...
tm_shape(landcover_ondo) +
  tm_raster(title = "Land cover categories") +
  tm_layout(frame = F, legend.position = c(0.8,0), title = "Ondo Study Sites") +
  tm_shape(ondo) +
  tm_polygons("white", alpha = 0.1) +
  tm_shape(ondo_sites_sf) +
  tm_dots(col = "name", size = 0.5, shape = 24, title = "Study Site", 
          palette = c("darkred", "darkblue"), border.col = NA)
...
tm_shape(afr %>% filter(ADM0_NAME %in% c("Burkina Faso", "Benin", "Cabo Verde", "C\xf4te d'Ivoire", "Gambia", "Ghana", "Guinea", 
  ...
  tm_polygons("white", alpha = 0.1, title = "West Africa") +
  tm_shape(afr %>% filter(ADM0_NAME %in% c("Gambia", "Nigeria"))) +
  tm_polygons(col = "ADM0_NAME", palette = c("#084887", "#909CC2"), border.col = "black", title = "SENZOR Countries") +
  tm_layout(frame = F, legend.title.size = 1.2, legend.text.size = 1)

# Gambia
gmb_sites <- tibble(
  name = c("Sambel Kunda", "Njoben"),
  lat = c(13.664998925851586, 13.4829019593275),
  long = c(-14.976053186600272, -14.944873714068498))

gmb_sites_sf <- gmb_sites %>% st_as_sf(coords = c("long", "lat"), crs = 4326)

tm_shape(gm_adm1) +
  tm_polygons("white", alpha = 0.1) +
  tm_shape(gm_adm2 %>% filter(ADM1_EN %in% c("Central River North", "Central River South"))) +
  tm_polygons(col = "ADM1_EN", palette = c("#084887", "#909CC2"), border.col = "black", title = "Region") +
  tm_shape(gmb_sites_sf) +
  tm_dots(col = "name", size = 0.5, shape = 24, title = "Study Site", palette = c("#e65100", "#ffc107"), border.col = "black") +
  tm_layout(frame = F, legend.title.size = 1.2, legend.text.size = 1) +
...
tm_shape(ng_adm1) +
  tm_polygons("white", alpha = 0.1) +
  tm_shape(ng_adm2 %>% filter(NAME_1 %in% c("Ebonyi", "Ondo"))) +
  tm_polygons(col = "NAME_1", palette = c("#084887", "#909CC2"), border.col = "black", title = "State") +
  tm_shape(ebonyi_sites_sf) +
  tm_dots(col = "name", size = 0.3, shape = 24, title = "Ebonyi Sites", palette = c("#e65100", "#ffc107"), border.col = "black") +
  tm_shape(ondo_sites_sf) +
  tm_dots(col = "name", size = 0.3, shape = 24, title = "Ondo Sites",  palette = c("#e65100", "#ffc107"), border.col = "black") +
  tm_layout(frame = F, legend.title.size = 1.2, legend.text.size = 1) +
